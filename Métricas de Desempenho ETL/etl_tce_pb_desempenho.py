...
DB_NAME = os.getenv("DB_NAME", "tce_pb_teste")
...
def carregar_dimensao_e_retornar_ids(engine, df: pd.DataFrame, table_name: str, 
    ...
    if not novos_registros.empty:
        ...
        # Para o cronômetro e calcula a diferença
        fim_tempo = time.time()
        tempo_decorrido = fim_tempo - inicio_tempo
        
        logging.info(f"Tempo de carga da tabela dimensão '{table_name}': {tempo_decorrido:.4f} segundos")
...
def processar_licitacoes(engine, df_licitacoes: pd.DataFrame):
    ...
    # Cronometrando a carga da tabela fato Licitacao
    inicio_tempo = time.time()
    df_fato.to_sql(name='licitacao', con=engine, if_exists='append', index=False, chunksize=10000)
    fim_tempo = time.time()
    
    logging.info(f"Carga de {len(df_fato)} registros de LICITAÇÕES concluída em {fim_tempo - inicio_tempo:.2f} segundos.")
...
def processar_despesas(engine, df_despesas: pd.DataFrame):
    ...
    # Cronometrando a carga da tabela fato Empenho
    inicio_tempo = time.time()
    df_fato.to_sql(name='empenho', con=engine, if_exists='append', index=False, chunksize=50000)
    fim_tempo = time.time()
    
    logging.info(f"Carga de {len(df_fato)} registros de DESPESAS (EMPENHOS) concluída em {fim_tempo - inicio_tempo:.2f} segundos.")
