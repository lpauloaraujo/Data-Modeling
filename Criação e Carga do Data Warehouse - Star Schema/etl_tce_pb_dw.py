...
Modelo dimensional:
  - 1 Fato:       fato_empenho
  - 7 Dimensoes:  dim_tempo, dim_municipio, dim_estrutura_administrativa,
                  dim_fornecedor, dim_programa_acao, dim_fonte_recurso,
                  dim_licitacao

Pre-requisitos:
  1. SCHEMA_DW.sql executado (cria `tce_pb_dw` com tabelas vazias + sentinel rows)
...
URI_OLTP = f"mysql+pymysql://{DB_USER}:{DB_PASS}@{DB_HOST}:{DB_PORT}/{SCHEMA_OLTP}"
URI_DW   = f"mysql+pymysql://{DB_USER}:{DB_PASS}@{DB_HOST}:{DB_PORT}/{SCHEMA_DW}"
...
def truncar_tabelas_dw(engine):
    ...
    tabelas = ['fato_empenho', 'dim_tempo', 'dim_municipio',
               'dim_estrutura_administrativa', 'dim_fornecedor',
...
def inserir_sentinel_rows(engine):
    ...
    statements = [
        ...
        "INSERT INTO dim_municipio (sk_municipio, nome_municipio) VALUES (-1, 'NAO INFORMADO')",
        """INSERT INTO dim_estrutura_administrativa
           ...
           VALUES (-1, 'ND', 'NAO INFORMADO', 'ND', 'NAO INFORMADO')""",
        "INSERT INTO dim_fornecedor (sk_fornecedor, cpf_cnpj, nome, tipo_pessoa) VALUES (-1, '00000000000', 'NAO INFORMADO', 'ND')",
        """INSERT INTO dim_programa_acao
             (sk_programa_acao, codigo_programa, descricao_programa, codigo_acao, descricao_acao)
           VALUES (-1, 'ND', 'NAO INFORMADO', 'ND', 'NAO INFORMADO')""",
        "INSERT INTO dim_fonte_recurso (sk_fonte_recurso, codigo_fonte_recurso, descricao_fonte_recurso, ano_fonte) VALUES (-1, 'ND', 'NAO INFORMADO', 'ND')",
        """INSERT INTO dim_licitacao
           ...
           VALUES (-1, 'SEM LICITACAO', 'SEM LICITACAO', NULL, 'SEM LICITACAO', NULL, NULL)""",
    ]
    with engine.begin() as conn:
...
def carregar_dim_municipio(engine_oltp, engine_dw):
    logging.info("Carregando dim_municipio...")
    df = pd.read_sql(f"SELECT nome_municipio FROM {SCHEMA_OLTP}.municipio", engine_oltp)
    df.to_sql('dim_municipio', engine_dw, if_exists='append', index=False)
    logging.info(f"  {len(df)} municipios inseridos")


def carregar_dim_estrutura_administrativa(engine_oltp, engine_dw):
    ...
    sql = f"""
        SELECT DISTINCT
            ...
            uo.codigo_unidade_orcamentaria,
            uo.descricao_unidade_orcamentaria
    ...
    """
    df = pd.read_sql(sql, engine_oltp)
    df.to_sql('dim_estrutura_administrativa', engine_dw, if_exists='append', index=False, chunksize=1000)
    logging.info(f"  {len(df)} combinacoes UG x UO inseridas")


def carregar_dim_fornecedor(engine_oltp, engine_dw):
    ...
    df = pd.read_sql(f"SELECT cpf_cnpj, nome FROM {SCHEMA_OLTP}.fornecedor", engine_oltp)

    def classificar(cpf_cnpj):
        if pd.isna(cpf_cnpj):
            return 'ND'
        s = ''.join(filter(str.isdigit, str(cpf_cnpj)))
        if len(s) == 11:
            return 'PF'
        if len(s) == 14:
            return 'PJ'
    ...
    df['tipo_pessoa'] = df['cpf_cnpj'].apply(classificar)
    df.to_sql('dim_fornecedor', engine_dw, if_exists='append', index=False, chunksize=5000)
    logging.info(f"  {len(df)} fornecedores inseridos (PF/PJ/ND classificados)")


def carregar_dim_programa_acao(engine_oltp, engine_dw):
    ...
    """
    df = pd.read_sql(sql, engine_oltp)
    df.to_sql('dim_programa_acao', engine_dw, if_exists='append', index=False, chunksize=5000)
    logging.info(f"  {len(df)} combinacoes Programa x Acao inseridas")


def carregar_dim_fonte_recurso(engine_oltp, engine_dw):
    logging.info("Carregando dim_fonte_recurso...")
    sql = f"SELECT codigo_fonte_recurso, descricao_fonte_recurso, ano_fonte FROM {SCHEMA_OLTP}.fonte_recurso"
    df = pd.read_sql(sql, engine_oltp)
    df.to_sql('dim_fonte_recurso', engine_dw, if_exists='append', index=False, chunksize=1000)
    logging.info(f"  {len(df)} fontes de recurso inseridas")


def carregar_dim_licitacao(engine_oltp, engine_dw):
    """
    ...
    """
    df = pd.read_sql(sql, engine_oltp)
    df.to_sql('dim_licitacao', engine_dw, if_exists='append', index=False, chunksize=5000)
    logging.info(f"  {len(df)} licitacoes inseridas (modalidade='NAO INFORMADO' por padrao)")
...
def carregar_fato_empenho(engine_dw):
    """
    Carrega fato_empenho via SQL JOIN cross-schema dentro do MySQL.
    Muito mais rapido do que trazer linhas pro Python e re-inserir.
    Usa o operador <=> (NULL-safe equal) nas chaves com NULL possivel.
    """
    ...
    SELECT
        ...
        e.numero_empenho                                          AS numero_empenho,
        COALESCE(o.numero_obra, 'SEM OBRA')                       AS numero_obra,
        ...
        COALESCE(e.valor_empenhado, 0) - COALESCE(e.valor_pago, 0) AS saldo_a_pagar
    FROM       {SCHEMA_OLTP}.empenho                        e
    LEFT JOIN  {SCHEMA_DW}.dim_tempo                        dt
            ON dt.data = e.data_empenho
    LEFT JOIN  {SCHEMA_OLTP}.municipio                      m
            ON e.municipio_id = m.id_municipio
    LEFT JOIN  {SCHEMA_DW}.dim_municipio                    dm
            ON dm.nome_municipio = m.nome_municipio
    ...
    LEFT JOIN  {SCHEMA_DW}.dim_estrutura_administrativa     de
            ON de.codigo_unidade_gestora      <=> ug.codigo_unidade_gestora
           AND de.codigo_unidade_orcamentaria <=> uo.codigo_unidade_orcamentaria
    LEFT JOIN  {SCHEMA_OLTP}.fornecedor                     f
            ON e.credor_id = f.id_fornecedor
    LEFT JOIN  {SCHEMA_DW}.dim_fornecedor                   df
            ON df.cpf_cnpj <=> f.cpf_cnpj
    ...
    LEFT JOIN  {SCHEMA_DW}.dim_programa_acao                dpa
            ON dpa.codigo_programa <=> p.codigo_programa
           AND dpa.codigo_acao     <=> a.codigo_acao
    LEFT JOIN  {SCHEMA_OLTP}.fonte_recurso                  fr
            ON e.fonte_recurso_id = fr.id_fonte_recurso
    LEFT JOIN  {SCHEMA_DW}.dim_fonte_recurso                dfr
            ON dfr.codigo_fonte_recurso <=> fr.codigo_fonte_recurso
           AND dfr.ano_fonte            <=> fr.ano_fonte
    LEFT JOIN  {SCHEMA_OLTP}.licitacao                      l
            ON e.licitacao_id = l.id_licitacao
    LEFT JOIN  {SCHEMA_DW}.dim_licitacao                    dl
            ON dl.numero_licitacao     <=> l.numero_licitacao
           AND dl.numero_protocolo_tce <=> l.numero_protocolo_tce
    LEFT JOIN  {SCHEMA_OLTP}.obra                           o
            ON e.obra_id = o.id_obra
    """

    with engine_dw.begin() as conn:
        conn.execute(text(sql))
    logging.info("  fato_empenho carregada.")
...
def imprimir_resumo(engine_dw):
    ...
    tabelas = ['dim_tempo', 'dim_municipio', 'dim_estrutura_administrativa',
               'dim_fornecedor', 'dim_programa_acao', 'dim_fonte_recurso',
    ...
    with engine_dw.connect() as conn:
        ...
        row = conn.execute(text("""
            SELECT
              ...
              SUM(sk_municipio        = -1) AS sem_municipio,
        ...
        if row:
            ...
            logging.info(f"  sem municipio:   {int(row[1] or 0):>15,}")
            logging.info(f"  sem estrutura:   {int(row[2] or 0):>15,}")
            logging.info(f"  sem fornecedor:  {int(row[3] or 0):>15,}")
            logging.info(f"  sem prog/acao:   {int(row[4] or 0):>15,}")
            logging.info(f"  sem fonte:       {int(row[5] or 0):>15,}")
            logging.info(f"  sem licitacao:   {int(row[6] or 0):>15,}  (esperado: dispensas/inexigibilidades)")
...
def executar_pipeline(enriquecer_modalidade: bool = False, diretorio_csv: str = None):
    ...
    try:
        engine_oltp = create_engine(URI_OLTP, pool_recycle=3600)
        engine_dw   = create_engine(URI_DW,   pool_recycle=3600)
        with engine_oltp.connect() as conn:
            conn.execute(text("SELECT 1"))
        with engine_dw.connect() as conn:
            conn.execute(text("SELECT 1"))
        logging.info("Conexoes OLTP e DW estabelecidas.")
    ...
    carregar_dim_municipio(engine_oltp, engine_dw)
    carregar_dim_estrutura_administrativa(engine_oltp, engine_dw)
    carregar_dim_fornecedor(engine_oltp, engine_dw)
    carregar_dim_programa_acao(engine_oltp, engine_dw)
    carregar_dim_fonte_recurso(engine_oltp, engine_dw)
    carregar_dim_licitacao(engine_oltp, engine_dw)
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("=== ETL TCE-PB: OLTP -> DATA WAREHOUSE (STAR SCHEMA) ===")
    print("=" * 60)
    print("Origem : tce_pb     (OLTP relacional ja carregado)")
    print("Destino: tce_pb_dw  (Star Schema: 1 fato + 7 dimensoes)")
    print("=" * 60)

    resp = input("\nEnriquecer modalidade de licitacao lendo os CSVs? [s/N]: ").strip().lower()
    enriquecer = resp in ('s', 'sim', 'y', 'yes')

    dir_csv = None
    if enriquecer:
        padrao = "../cargas"
        dir_csv = input(f"Diretorio dos CSVs licitacoes-*.csv [padrao: {padrao}]: ").strip() or padrao
        if not os.path.isdir(dir_csv):
            logging.warning(f"Diretorio '{dir_csv}' nao existe -- pulando enriquecimento.")
            enriquecer = False

    executar_pipeline(enriquecer_modalidade=enriquecer, diretorio_csv=dir_csv)
